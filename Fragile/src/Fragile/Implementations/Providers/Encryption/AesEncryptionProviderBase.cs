...
internal abstract class AesEncryptionProviderBase : IEncryptionProvider
{
    ...
    public async Task EncryptAsync(Stream source, Stream target, EncryptionOptions options, CancellationToken cancellationToken = default)
    {
        ...
        using CryptoStream cryptoStream = new(target, aes.CreateEncryptor(), CryptoStreamMode.Write, leaveOpen: true);
    ...
    public async Task DecryptAsync(Stream source, Stream target, EncryptionOptions options, CancellationToken cancellationToken = default)
    {
        ...
        using CryptoStream cryptoStream = new(source, aes.CreateDecryptor(), CryptoStreamMode.Read, leaveOpen: true);
    ...
    {
        Aes? aes = Aes.Create(); // Creates a new AES instance (defaults might vary, e.g., AesCryptoServiceProvider)
        if (aes is null)
        {
    ...
    {
        byte[] iv = new byte[DefaultIvSizeBytes];
        using (RandomNumberGenerator rng = RandomNumberGenerator.Create())
        {
            rng.GetBytes(iv);
        }
    ...
    private byte[] DeriveKeyFromPassword(string password, byte[] salt)
    {
        ...
        using Rfc2898DeriveBytes kdf = new(password, salt, DefaultPbkdf2Iterations, DefaultPbkdf2HashAlgorithm);
