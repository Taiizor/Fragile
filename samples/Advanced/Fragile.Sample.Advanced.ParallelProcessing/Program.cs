...
namespace Fragile.Sample.Advanced.ParallelProcessing
{
    class Program
    {
        ...
        static async Task RunComparisonBenchmarks(string directory)
        {
            ...
            var benchmarks = new List<BenchmarkConfig>
            {
                new BenchmarkConfig
                {
            ...
            {
                var result = await RunBenchmark(testDataDir, resultsDir, config);
                results.Add(result);
            }
        ...
        static void DisplayBenchmarkResults(List<BenchmarkResult> results)
        {
            ...
            {
                var sequential = results.FirstOrDefault(r => !r.UseParallelProcessing && r.CompressionLevel == CompressionLevel.Normal);
                var parallel = results.FirstOrDefault(r => r.UseParallelProcessing && r.CompressionLevel == CompressionLevel.Normal && r.MaxThreads == Environment.ProcessorCount);
