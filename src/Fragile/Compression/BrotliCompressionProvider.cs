...
{
    /// <summary>
    /// Compression provider implementation using Brotli algorithm
    /// </summary>
    /// <remarks>
    /// Creates a new Brotli compression provider with the specified level and parallel processing options
    /// </remarks>
    ...
    internal class BrotliCompressionProvider(CompressionLevel level, bool useParallelProcessing, int maxThreads) : CompressionProvider(useParallelProcessing, maxThreads)
    {
        /// <summary>
        ...
        /// <summary>
        /// Creates a new Brotli compression provider with the specified level
        /// </summary>
        /// <param name="level">Compression level</param>
        public BrotliCompressionProvider(CompressionLevel level) : this(level, true, Environment.ProcessorCount)
        {
        }

        /// <summary>
        /// Compresses the input stream to the output stream using Brotli
        /// </summary>
        public override async Task<long> CompressAsync(Stream input, Stream output, IProgress<double>? progress = null, CancellationToken cancellationToken = default)
        {
            ...
            {
                using BrotliStream brotliStream = new(output, compressionLevel, true);
                ...
                {
#if NET48_OR_GREATER || NETSTANDARD2_0
                    await brotliStream.WriteAsync(buffer, 0, bytesRead, cancellationToken).ConfigureAwait(false);
#else
                    await brotliStream.WriteAsync(buffer.AsMemory(0, bytesRead), cancellationToken).ConfigureAwait(false);
#endif
...
#else
            throw new NotSupportedException("Brotli compression is not supported in this framework version. Please use .NET Standard 2.1, .NET 6.0 or higher.");
#endif
        }

        /// <summary>
        /// Decompresses the input stream to the output stream using Brotli
        /// </summary>
        public override async Task<long> DecompressAsync(Stream input, Stream output, IProgress<double>? progress = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
...
#if NET48_OR_GREATER || NETSTANDARD2_0
                                        while (totalBytesRead < length && (bytesRead = await brotliStream.ReadAsync(buffer, 0, (int)Math.Min(buffer.Length, length - totalBytesRead), cancellationToken)) > 0)
#else
                                        while (totalBytesRead < length && (bytesRead = await brotliStream.ReadAsync(buffer.AsMemory(0, (int)Math.Min(buffer.Length, length - totalBytesRead)), cancellationToken)) > 0)
#endif
                                        {
#if NET48_OR_GREATER || NETSTANDARD2_0
                ...
                using (BrotliStream brotliStream = new(input, CompressionMode.Decompress, true))
                {
                    byte[] buffer = new byte[81920]; // 80 KB buffer
                    long totalBytes = 0;
                    int bytesRead;
...
#if NET48_OR_GREATER || NETSTANDARD2_0
                    while ((bytesRead = await brotliStream.ReadAsync(buffer, 0, buffer.Length, cancellationToken).ConfigureAwait(false)) > 0)
#else
            ...
            catch (Exception ex)
            {
                ...
                throw new InvalidDataException($"Failed to decompress using Brotli algorithm: {ex.Message}", ex);
            }

            // Return the number of bytes written
            return output.Position - initialPosition;
#else
            throw new NotSupportedException("Brotli decompression is not supported in this framework version. Please use .NET Standard 2.1, .NET 6.0 or higher.");
#endif
        }

        /// <summary>
        /// Estimates the compressed size based on the Brotli algorithm's average compression ratio
        /// </summary>
        public override long EstimateCompressedSize(long inputSize)
        {
            // Brotli typically achieves a compression ratio of about 2:1 to 3:1 for text
        ...
        private async Task CompressParallelAsync(Stream input, Stream output, System.IO.Compression.CompressionLevel compressionLevel, IProgress<double>? progress = null, CancellationToken cancellationToken = default)
        {
            ...
            for (long position = 0; position < fileLength; position += chunkSize)
            {
                ...
                compressionTasks.Add(Task.Run(async () =>
                {
                    ...
                    using (BrotliStream brotliStream = new(chunkOutput, compressionLevel, true))
                    {
#if NET48_OR_GREATER || NETSTANDARD2_0
                        await brotliStream.WriteAsync(chunkBuffer, 0, (int)length, cancellationToken);
#else
                        await brotliStream.WriteAsync(chunkBuffer.AsMemory(0, (int)length), cancellationToken);
#endif
                    }
            ...
            progress?.Report(1.0);
#else
            throw new NotSupportedException("Parallel compression is not supported in this framework version. Please use .NET Standard 2.1, .NET 6.0 or higher.");
#endif
        }
    }
}